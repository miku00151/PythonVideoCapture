...
class CaptureCardViewer:
    def __init__(self, root):
        ...
        self.start_button = tk.Button(root, text="開啟影像", command=self.create_capture_window)
    ...
    def create_capture_window(self):
        self.new_capture_window = threading.Thread(target=self.start_capture,daemon=True)
        self.new_capture_window.start()

    def set_attr_disable(self):
        self.device_combobox.config(state="disable")
        self.resolution_combobox.config(state="disable")
        self.fps_combobox.config(state="disable")
        self.start_button.config(state="disable")

    def set_attr_enable(self):
        self.device_combobox.config(state="normal")
        self.resolution_combobox.config(state="normal")
        self.fps_combobox.config(state="normal")
        self.start_button.config(state="normal")

    def close_capture_window(self):
        self.set_attr_enable()
